...
## 3. Typography
- **Fraunces** (Bold for the logotype; 500 elsewhere) — the logotype, plus display use everywhere else: headlines, tagline. One typeface now covers both jobs, so the wordmark and the rest of the brand's own "voice" type feel like one family.
- **Public Sans** (body/UI) — everything the user reads to get work done.
- **IBM Plex Mono** (utility) — Health Score, deltas ("68 ▲ +7"), counts, timestamps.
