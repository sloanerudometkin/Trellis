...
export function OverviewView({ analysis, latestReport, reportLoading, analysisError, onAnalyze, onRetry, analysisStarting = false }: {
  ...
  if (!analysis) return <section className="empty-state" aria-labelledby="analysis-title" aria-busy={analysisStarting}><p className="eyebrow">Website analysis</p><h2 id="analysis-title" className="mt-3 font-display text-2xl">Turn your website into a starting strategy.</h2><p className="mt-3 max-w-xl leading-7 text-ink/70">Trellis will safely read up to 15 public pages, generate one connected paid-and-organic plan, and save a Report.</p>{analysisError && <div className="error-box" role="alert">{analysisError}</div>}<button className="primary-button sm:w-auto" disabled={analysisStarting} onClick={onAnalyze}>{analysisStarting ? "Starting analysis…" : "Analyze website"}</button></section>;
  ...
  return <section className="mt-7" data-testid="analysis-results" aria-label="Overview summary">
    ...
    <div className="overview-callout"><div><p className="eyebrow">Last analysis</p><h2 className="mt-2 font-display text-2xl">{analysis.completed_at ? new Date(analysis.completed_at).toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" }) : "Completed"}</h2><p className="mt-2 text-sm text-ink/70">{analysis.pages_scanned_count} {analysis.pages_scanned_count === 1 ? "page" : "pages"} analyzed</p></div><button className="secondary-button sm:w-auto" onClick={onAnalyze}>Rescan website</button></div>
