...
**How it works:** The user can analyze and rescan a website whenever needed. Every run produces connected paid-and-organic opportunities, strengthens one evolving strategy and prioritized plan, carries accepted SEO, AEO, content, technical, and SEM recommendations through one Organizer, and automatically saves a Report that builds the website's progress history.
...
- **FR-3.1** The system shall analyze scraped site content for word frequency and rank keyword and keyword-phrase candidates (unigrams through trigrams, e.g. single words like 'marketing' as well as multi-word long-tail phrases like 'content marketing strategy') by frequency and trend score.
...
- **FR-5.2** The agent shall orchestrate, in order: scrape → word-frequency/keyword analysis (including the FR-3.5 TF-IDF boilerplate filter, when multiple pages were scraped) → trend lookup → best-practice retrieval → technical audit (FR-6) → generate suggestions (single structured-output LLM call) → compute Health Score (FR-7) → generate Report (FR-9) → persist results.
...
| PostgreSQL (Neon/Supabase) | Internal | Primary data store for all entities (DR-1–DR-14) | Connection credentials | Also stores raw scrape snapshots; pgvector added Phase 2 |
...
**Phase 1 — MVP (target: ~19.5–20 weeks, Sep 2026 → early Jan 2027)**

| Milestone | Covers | Est. duration |
|---|---|---|
| Setup & architecture | Repo scaffolding, DB schema (DR-1–DR-14), auth (NFR-1), CI/CD | 2 weeks |
| Scrape + keyword pipeline | FR-3.1, FR-3.5, scraping stack, word-frequency/keyword ranking, scikit-learn TF-IDF boilerplate filter | 2 weeks |
...
| Technical audit | FR-6 (including FR-6.7 TF-IDF near-duplicate check), PageSpeed API integration | 2 weeks |
| SEM strategy engine | FR-8, heuristic Cost Tier logic, ad-group/negative-keyword generation, SEM tab UI | 2 weeks |
| Health Score + delta + polish | FR-7, since-last-scan delta, accessibility pass (NFR-4) | 2 weeks |
| Reports & KPI comparison | FR-9, Report generation on every AnalysisRun, Report History view, Compare view, predefined-KPI aggregation | 1.5–2 weeks |
| Testing & deploy | End-to-end testing, free-tier hosting deploy, bug fixing | 2 weeks |

**Phase 2 — Grow (target: ~11.5–12 weeks, early Jan 2027 → late March 2027)**
...
**Phase 3 — Scale (target: ~10 weeks, late March 2027 → early June 2027)**
