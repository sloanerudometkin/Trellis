...
Software Requirements Specification (v2.7 — September 4, 2026)
...
|---|---|---|
| Sep 4 | Foundation and contracts | Repo/app scaffolding, environment configuration, database schema (DR-1–DR-14) via SQLAlchemy/Alembic, Supabase Auth/RLS baseline (NFR-1), and CI build checks |
| Sep 5–6 | First end-to-end analysis path | URL submission and workspace (FR-1), safe HTTPX/BeautifulSoup scrape, keyword extraction/TF-IDF filtering (FR-3), persisted resumable `AnalysisRun`, and visible results |
| Sep 7 | AI recommendation pipeline | Plain-Python staged pipeline and Pydantic validation (FR-5), Groq integration/fallback behavior, AEO and SEO/content recommendations (FR-2–FR-3) |
| Sep 8 | Action workflow | Accept/Dismiss behavior, task creation, and Organizer/Kanban stages (FR-4) connected to real persisted suggestions |
| Sep 9 | Technical audit | FR-6 checks, PageSpeed integration, near-duplicate detection, and technical findings UI |
| Sep 10 | SEM strategy | FR-8 keyword/ad-group/placement/negative-keyword suggestions, disclosed heuristic Cost Tier, and SEM summary UI |
| Sep 11 | Measurement | Health Score and since-last-scan delta (FR-7); automatic Report snapshots, history, and comparison (FR-9) |
| Sep 12 | Complete integrated UI | All six workspace views connected to the same website and analysis data; loading, empty, failure, and retry states present |
| Sep 13 | Test, accessibility, and deploy | Critical pytest/Vitest/Playwright coverage, accessibility pass (NFR-4), production configuration, Render backend deployment, and Netlify frontend deployment |
| Sep 14 | Final acceptance and demo | Production smoke test on a representative public website, blocker fixes, repeat analysis/rescan verification, and a rehearsed end-to-end demo |
...
**Daily control point:** at the end of each day, verify the scheduled outcome in the running app. Any incomplete blocker becomes the first task the following morning. No new feature may displace a critical-path FR-1–FR-9 requirement before final acceptance.
