...
Software Requirements Specification (v2.8 — September 4, 2026)
...
- **FR-9.2** Each Report shall be built from a **predefined KPI set** (not user-configurable in MVP, so a marketer never has to decide which metrics matter) spanning:
  - *Organic:* Health Score and its change since the prior report (FR-7); AEO checklist completion % and its change (FR-2); technical audit findings resolved vs. still open (FR-6); count of content items reaching Published this period (FR-4); top keyword/content-gap candidates by trend score (FR-3.1).
...
**Fixed deadline:** the complete Phase 1 MVP (FR-1 through FR-9) must be deployed and demo-ready by **Monday, September 14, 2026**. This replaces the earlier part-time estimate that extended into January 2027.

**Capacity assumption:** September 4–14 is an intensive, deadline-driven sprint for one developer, not a 5–10-hours/week schedule. The feature scope is frozen: no Phase 2/3 work or new MVP features enter this sprint. Work is organized as a thin end-to-end product path first, then completed and hardened in priority order. Because the prior bottom-up estimate was ~19.5–20 part-time weeks, this deadline carries substantial delivery risk and requires daily scope/status checks.

**Test-first rule:** automated tests are part of every milestone from the first day, not a final cleanup task. Before implementing each user case, write its expected behavior as a failing test, implement the smallest working behavior, then rerun the relevant unit and integration tests until they pass. **Unit tests** verify one small function or component in isolation; **integration tests** verify that connected parts (such as the API, database, scraper, and LLM boundary) work together. Playwright end-to-end tests cover only the most important complete user journeys because of the compressed deadline.

**Phase 1 — MVP (fixed target: Sep 4–14, 2026)**

| Date | Milestone | Required outcome, including tests written first |
|---|---|---|
| Sep 4 | Foundation, test harness, and contracts | Configure pytest, Vitest, and Playwright; establish test folders, fixtures/mocks, a separate test database, and CI test commands before feature work. Then scaffold the app and implement database (DR-1–DR-14), Supabase Auth/RLS (NFR-1), and API-contract foundations against the first unit/integration tests. |
| Sep 5–6 | First end-to-end analysis path | First write user-case and integration tests for URL validation, workspace creation, safe scraping, failed/retried analysis, persisted status, and visible results. Then implement FR-1, FR-3, and the first critical Playwright journey: sign in → add website → analyze → view results. |
| Sep 7 | AI recommendation pipeline | First write unit tests for pipeline stages, prompt-input preparation, Pydantic validation, retry/fallback behavior, and mocked LLM responses; add an integration test for recommendations saved to the database. Then implement FR-2, FR-3, and FR-5. |
| Sep 8 | Action workflow | First write tests for Accept/Dismiss decisions, dismiss reasons, task creation, valid Organizer stage changes, persistence, and user isolation. Then implement FR-4 and extend the Playwright journey through the Organizer. |
| Sep 9 | Technical audit | First write unit/integration tests for each FR-6 rule, PageSpeed success/failure handling, and TF-IDF near-duplicate thresholds using fixed fixtures. Then implement the audit and findings UI. |
| Sep 10 | SEM strategy | First write tests for FR-8 suggestion fields, deterministic heuristic Cost Tier boundaries, negative keywords, disclosure text, and saved SEM output. Then implement the SEM engine and UI. |
| Sep 11 | Measurement | First write tests for Health Score calculations/deltas, Report creation on every completed analysis, immutable snapshots, KPI aggregation, and report comparison. Then implement FR-7 and FR-9. |
| Sep 12 | Complete integrated UI | First write component and integration tests for all six views and their loading, empty, failure, and retry states. Then connect every view to the same website/analysis data and finish the critical Playwright rescan/report-comparison journey. |
| Sep 13 | Full regression, accessibility, and deploy | Run the complete test suite written throughout the sprint; fix regressions instead of introducing the first tests. Complete accessibility checks (NFR-4), production configuration, Render/Netlify deployment, and post-deploy smoke tests. |
| Sep 14 | Final acceptance and demo | Run the full suite and production smoke checklist, fix blockers, verify both critical Playwright journeys against the deployed app, and rehearse the end-to-end demo. |

**Definition of done for September 14:** a user can sign in, add a website, run and monitor an analysis, view SEO/AEO/technical/SEM recommendations, accept or dismiss recommendations, move accepted work through the Organizer, view the Health Score, rescan the website, and view/compare saved Reports in the deployed app. Required security controls in NFR-1 and critical-path automated tests must pass. Phase 2/3 features and nonessential visual refinements are not part of this deadline.
...
**Phase 2 — Grow (schedule to be re-estimated after the Sep 14 MVP)**
...
- **September 14 deadline compression** — the single biggest schedule risk. The complete MVP was previously estimated at ~19.5–20 part-time weeks and is now scheduled as an intensive September 4–14 sprint for one developer. The deadline is fixed, so daily acceptance checks, a strict feature freeze, and immediate escalation of blockers are required; the schedule change does not make the underlying work smaller.
