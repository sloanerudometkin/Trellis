...
|---|---|---|
| Setup & architecture | Repo scaffolding, DB schema (DR-1–DR-14) via SQLAlchemy/Alembic, Supabase Auth (NFR-1), GitHub Actions CI | 2 weeks |
| Scrape + keyword pipeline | FR-3.1, FR-3.5, HTTPX/BeautifulSoup scraping stack (Playwright fallback), word-frequency/keyword ranking, scikit-learn TF-IDF boilerplate filter | 2 weeks |
| AI agent orchestration | FR-5.1–5.6, plain-Python pipeline (no LangGraph — see 2.5), Pydantic schema validation, LLM integration, resumable `AnalysisRun` stages (NFR-2) | 3 weeks |
...
| Testing & deploy | pytest (backend) + Vitest (frontend) + Playwright (end-to-end) test suites, Render/Netlify free-tier deploy, bug fixing | 2 weeks |
